...
const DefaultDashboardTemplate = `{{define "counter_gauge_cumulative"}}
{
  ...
  "spaceLength": 10,
  "stack": false,
  "targets": [{"expr": "{{ .AggregationFunc }}({{ .FullMetricName }}){{ .MetricLabels }}", "intervalFactor": 1, "refId": "A"{{ if .LegendFormat }}, "legendFormat": "{{ .LegendFormat }}"{{ end }}}],
  "thresholds": [],
  "timeFrom": null,
  "timeRegions": [],
  "timeShift": null,
...
{{define "counter_gauge_rate"}}
{
  ...
  "spaceLength": 10,
  "stack": false,
  "targets": [{"expr": "{{ .AggregationFunc }}(rate({{ .FullMetricName }}[15m])){{ .MetricLabels }}", "intervalFactor": 1, "refId": "A"{{ if .LegendFormat }}, "legendFormat": "{{ .LegendFormat }}"{{ end }}}],
  "thresholds": [],
  "timeFrom": null,
  "timeRegions": [],
  "timeShift": null,
...
{{define "errors"}}
{
  ...
  "spaceLength": 10,
  "stack": false,
  "targets": [{"expr": "sum({{ .MetricsPrefix }}errors) by (error_type)", "intervalFactor": 1, "refId": "A", "legendFormat": "{{"{{"}}error_type{{"}}"}}"}],
  "thresholds": [],
  "timeFrom": null,
  "timeRegions": [],
  "timeShift": null,
...
{{define "summary_timer"}}
{
  ...
  "spaceLength": 10,
  "stack": false,
  "targets": [{"expr": "avg({{ .FullMetricName }}{ {{- .ExtraLabelFilter }}quantile=~\"0.5|0.75|0.9|0.99\"}){{ .MetricLabels }}", "format": "time_series", "intervalFactor": 1, "refId": "A"{{ if .LegendFormat }}, "legendFormat": "{{ .LegendFormat }}"{{ end }}}],
  "thresholds": [],
  "timeFrom": null,
  "timeRegions": [],
  "timeShift": null,
